...
class RENDER_main_panel(Panel):
    ...
    def draw(self, context):
        layout = self.layout
        layout.operator(
            "freestyle_render_set_param_op.operator",
            icon='RENDER_STILL')
...
class FREESTYLE_RENDER_set_param_op(Operator):
    ...
    def execute(self, context):
        ...
        crease_angle_lineset.linestyle.alpha_modifiers["Crease Angle"].angle_min = 1.39626 # 80° in radian
        crease_angle_lineset.linestyle.alpha_modifiers["Crease Angle"].angle_max = 1.74533 # 100° in radian
...
class DISPLAY_COLOR_main_panel(Panel):
    ...
    def draw(self, context):
        ...
        layout.operator("display_color_reset_everything.operator", icon = "DECORATE_OVERRIDE")
    ...
    def execute(self, context):
        for mesh in [obj for obj in bpy.data.objects if obj.type == "MESH"]:
            for slot in mesh.material_slots:
                ...
                if background_node:
                    material.diffuse_color = background_node.inputs["Color"].default_value
        return {'FINISHED'}
